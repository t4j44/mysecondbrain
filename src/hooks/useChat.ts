 ...
 * Connected to `/api/analyze` by default.
...
export function useChat(options: UseChatOptions = {}): UseChatHelpers {
  ...
  const sendMessage = useCallback(
    async (customContent?: string) => {
      ...
      // Instantly render the User message and a pending Thinking Assistant message
      ...
      try {
        ...
        while (true) {
          ...
          // Split buffer into lines for SSE / Vercel AI protocol stream parsing
          ...
          for (const line of lines) {
            ...
            // Stream Protocol Parsing Logic:
            // 1. Vercel AI SDK Data-Stream format: 0:"text", e:citation, 3:error
            // 2. Standard SSE format: data: {"text": "..."} or data: "..."
            // 3. Fallback raw text chunk
            let textDelta = '';
            let citationPayload: Citation | null = null;
            let streamError: string | null = null;

            if (trimmedLine.startsWith('0:')) {
              // Text token chunk in Vercel Data-Stream
              try {
              ...
              // Direct unformatted text fallback
              textDelta = line;
            }
        ...
        // Process any remaining text in buffer if available
        if (buffer.trim()) {
        ...
        if (err.name === 'AbortError') {
          // Graceful handling of user-initiated cancellation
          setMessages((prev) =>
            prev.map((msg) =>
              msg.id === assistantMessageId
                ? {
