...
async def relevant_contacts(db, owner: str, query: str, limit: int = 5) -> list[dict]:
    try:
        evidence = await semantic_search(db, owner, query, 20, ['person', 'interaction', 'project', 'organization'])
    except (AIProviderError, PrivacyBlocked):
        evidence = []
    literal = await perform_keyword_search(db, owner, query, 20, ['person', 'interaction', 'project', 'organization'])
    ...
    for item in evidence:
        ...
        if item.entity_type == 'interaction':
            record = await owned_record(db, owner, 'interaction', item.id)
            if record.person_id:
                identities.add(str(record.person_id))
        if item.entity_type == 'project':
            identities.update(str(identity) for identity in (await db.execute(select(Interaction.person_id).where(
                Interaction.user_id == owner, Interaction.project_id == item.id,
...
async def structured_answer(db, owner: str, prompt: str) -> dict | None:
    ...
    if kind:
        records = (await db.execute(select(model).where(model.user_id == owner,
            model.deleted_at.is_(None), model.archived_at.is_(None), *conditions).order_by(order).limit(25))).scalars().all()
        lines, citations = [], []
