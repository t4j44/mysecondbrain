...
export default function SettingsOverviewPage() {
  ...
  return <div className="max-w-2xl space-y-6"><h1 className="text-2xl font-semibold">Settings and beta feedback</h1>
    ...
    <details className="rounded-xl border p-4"><summary className="cursor-pointer font-medium">Other saved tools</summary><div className="mt-3 flex flex-wrap gap-4">{[['Notes', '/memories'], ['Ideas', '/ideas'], ['Achievements', '/achievements'], ['Content', '/content'], ['Personal metrics', '/kpis']].map(([label, href]) => <Link key={href} href={href} className="min-h-11 py-2 underline">{label}</Link>)}</div></details>
