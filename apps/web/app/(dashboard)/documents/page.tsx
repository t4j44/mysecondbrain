...
export default function DocumentsPage() {
  ...
  return <div className="mx-auto max-w-3xl space-y-6">
    ...
    <p>Upload a PDF, text, or Markdown file. Text is extracted on the server; minimized text is sent for search indexing. For a photo or scanned PDF, add its readable text through <Link className="underline" href="/capture">Capture</Link>.</p>
    <form className="flex flex-wrap items-center gap-3 rounded-xl border p-4" onSubmit={event => {
      ...
      <label className="min-w-0 flex-1">Choose a file<input ref={fileInput} required type="file" accept=".pdf,.txt,.md" className="mt-2 block max-w-full text-sm" /></label>
    ...
    <ul className="space-y-3">{documents.map(doc => <li key={doc.id} className="space-y-3 rounded-xl border p-4">
      ...
      <p className="text-sm">Indexing: {doc.processing_status}{doc.error_state ? ` · ${doc.error_state}` : ''}</p>
