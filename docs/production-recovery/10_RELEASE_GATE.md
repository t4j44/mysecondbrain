...
| **BLK-REL-008** | **P1 (MCP)** | `apps/api/app/mcp/` (`server.py`, `tools.py`, `security.py`) | **Missing MCP Write Operations:** MCP server only implements read and draft scopes (`mcp:*:read`, `mcp:content:draft`). No mutating tools (such as `create_record` or `update_task`) exist in the MCP server. **RESOLVED at G5 (2026-08-28):** 11 write tools are registered behind granular `mcp:*:write` scopes — see [G5_MCP_FINALIZE_GATE.md](G5_MCP_FINALIZE_GATE.md). | **Integrations / MCP Engineer** |
...
| | Create Record | Fail / Blocked | **RESOLVED at G5** | 11 write tools are registered behind granular `mcp:*:write` scopes; read-only credentials receive 403. Live-database verification remains BLOCKED (no PostgreSQL). |
